...
export function RegisterPage() {
      ...
      {[1, 2, 3].map((s) => (
        <div key={s} className="flex items-center gap-2">
          <div className={`w-7 h-7 rounded-full flex items-center justify-center text-[11px] transition-all ${
            ...
            s === step ? "bg-[#6366F1] text-white" :
            "bg-[#D9D6D0] text-[#8C8980]"
          }`} style={{ fontWeight: 600 }}>
            {s < step ? <Check className="w-3.5 h-3.5" /> : s}
          </div>
          {s < 3 && <div className={`w-8 h-px ${s < step ? "bg-[#10B981]" : "bg-[#D9D6D0]"}`} />}
        </div>
      ))}
    </div>
  );
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-3 sm:px-4 py-6" style={{ background: "#F7F6F3" }}>
      ...
      <div className="w-full max-w-[420px] bg-white rounded-xl sm:rounded-2xl border border-[#D0D0E0] px-5 py-6 sm:p-8 shadow-sm">
        {stepIndicator}

        {/* Step 1: Account Info */}
        {step === 1 && (
          <>
            <h1 className="text-[20px] sm:text-[22px] text-[#1A1916] mb-1" style={{ fontWeight: 700 }}>Create your account</h1>
            <p className="text-[12px] sm:text-[13px] text-[#8C8980] mb-5 sm:mb-6">Start analyzing meetings in minutes</p>

            <div className="mb-3 sm:mb-4">
              <label className="text-[11px] sm:text-[12px] text-[#504E49] mb-1 sm:mb-1.5 block" style={{ fontWeight: 500 }}>Full Name</label>
              <input
                ...
                style={{ borderColor: errors.name ? "#F43F5E" : "#D0D0E0" }}
              />
            ...
            <div className="mb-3 sm:mb-4">
              <label className="text-[11px] sm:text-[12px] text-[#504E49] mb-1 sm:mb-1.5 block" style={{ fontWeight: 500 }}>Password</label>
            ...
            <p className="text-center text-[11px] sm:text-[12px] text-[#8C8980] mt-4 sm:mt-5">
              Already have an account?{" "}
              <button onClick={() => navigate("/login")} className="text-[#6366F1] hover:text-[#818CF8]" style={{ fontWeight: 500 }}>Sign in</button>
            </p>
          </>
        )}
        ...
        {step === 2 && (
          <div className="text-center py-2 sm:py-4">
            <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-full bg-[#6366F1]/10 flex items-center justify-center mx-auto mb-4">
              <Mail className="w-7 h-7 sm:w-8 sm:h-8 text-[#6366F1]" />
            </div>
            <h2 className="text-[18px] sm:text-[20px] text-[#1A1916] mb-2" style={{ fontWeight: 700 }}>Check your email</h2>
            <p className="text-[12px] sm:text-[13px] text-[#8C8980] mb-1">We sent a verification link to</p>
            <p className="text-[12px] sm:text-[13px] text-[#1A1916] mb-5 sm:mb-6" style={{ fontWeight: 500 }}>{email}</p>

            <button onClick={() => setStep(3)} className="w-full py-2 sm:py-2.5 rounded-md bg-[#6366F1] text-white text-[13px] hover:bg-[#818CF8] transition-all mb-3" style={{ fontWeight: 600 }}>
            ...
            <button
              ...
              className="text-[12px] text-[#6366F1] hover:text-[#818CF8] disabled:text-[#B8B5AE] transition-colors"
            >
        ...
        {step === 3 && (
          <>
            <h2 className="text-[18px] sm:text-[20px] text-[#1A1916] mb-1" style={{ fontWeight: 700 }}>Set up your workspace</h2>
            <p className="text-[12px] sm:text-[13px] text-[#8C8980] mb-5 sm:mb-6">You can always change this later</p>

            <div className="mb-3 sm:mb-4">
              <label className="text-[11px] sm:text-[12px] text-[#504E49] mb-1 sm:mb-1.5 block" style={{ fontWeight: 500 }}>Workspace Name</label>
              <input
                value={workspaceName} onChange={(e) => setWorkspaceName(e.target.value)}
                placeholder="Acme Corp"
                className="w-full bg-white border border-[#D0D0E0] rounded-md px-3 py-2 text-[13px] text-[#1A1916] placeholder-[#B8B5AE] outline-none focus:ring-2 focus:ring-[#6366F1]/20 focus:border-[#6366F1] transition-all"
              />
            </div>

            <div className="mb-5 sm:mb-6">
              <label className="text-[12px] text-[#504E49] mb-1.5 flex items-center gap-1.5" style={{ fontWeight: 500 }}>
                <Users className="w-3.5 h-3.5" /> Invite teammates <span className="text-[#B8B5AE]">(optional)</span>
              </label>
              <textarea
                value={inviteEmails} onChange={(e) => setInviteEmails(e.target.value)}
                placeholder="teammate@company.com, another@company.com"
                rows={2}
                className="w-full bg-white border border-[#D0D0E0] rounded-md px-3 py-2 text-[13px] text-[#1A1916] placeholder-[#B8B5AE] outline-none focus:ring-2 focus:ring-[#6366F1]/20 focus:border-[#6366F1] resize-none transition-all"
              />
            </div>

            <button onClick={() => navigate("/")}
              className="w-full py-2 sm:py-2.5 rounded-md bg-[#6366F1] text-white text-[13px] hover:bg-[#818CF8] transition-all" style={{ fontWeight: 600 }}>
              Launch ConvoCrystal
            </button>

            <button onClick={() => navigate("/")} className="w-full text-center mt-3 text-[11px] sm:text-[12px] text-[#8C8980] hover:text-[#504E49] transition-colors">
              Skip for now
            </button>
          </>
        )}
